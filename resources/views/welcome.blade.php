...
<section class="max-w-7xl mx-auto px-6 py-16">
    <div class="text-center mb-10">
        ...
        <p class="text-slate-500">Temukan event sesuai minat dan bidangmu</p>
    </div>
    @if($categories->count())
    <div class="flex flex-wrap justify-center gap-3">
        @foreach($categories as $category)
        <span class="px-6 py-3 bg-indigo-50 text-indigo-700 rounded-full font-bold text-sm border border-indigo-100 hover:bg-indigo-600 hover:text-white transition cursor-pointer">
            {{ $category->name }}
        </span>
        @endforeach
    </div>
    @else
    <p class="text-center text-slate-400 italic">Belum ada kategori tersedia.</p>
    @endif
</section>
...
<section class="max-w-7xl mx-auto px-6 py-16">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        ...
        <div class="flex flex-col items-center gap-3 p-5 rounded-2xl border border-slate-100 hover:shadow-md hover:border-indigo-200 transition group">
            @if($partner->logo_url)
                <img src="{{ $partner->logo_url }}" alt="{{ $partner->name }}"
                    class="w-16 h-16 object-contain group-hover:scale-110 transition-transform">
            @else
                <div class="w-16 h-16 rounded-2xl bg-indigo-100 flex items-center justify-center text-indigo-700 font-black text-2xl group-hover:bg-indigo-600 group-hover:text-white transition">
    ...
    @else
    <p class="text-center text-slate-400 italic">Belum ada partner terdaftar.</p>
    @endif
</section>
